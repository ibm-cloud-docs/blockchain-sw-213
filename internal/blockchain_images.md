...
docker pull cp.icr.io/cp/ibp-ccenv:1.4.6-20200416-amd64
docker pull cp.icr.io/cp/ibp-goenv:1.4.6-20200416-amd64
docker pull cp.icr.io/cp/ibp-nodeenv:1.4.6-20200416-amd64
```
{:codeblock}
...
3. If you are deploying a peer node, you need to use the core chaincode variables to instruct the peer to build chaincode using the {{site.data.keyword.blockchainfull_notm}} certified images. For example, if you are using Go chaincode, you need to set the following variables:
  ```
  CORE_CHAINCODE_NODE_RUNTIME=cp.icr.io/cp/ibp-nodeenv:1.4.6-20200416-amd64
...
services:
  peer-base:
    image: cp.icr.io/cp/ibp-peer:1.4.6-20200416-amd64
    environment:
      - LICENSE=accept
      - FABRIC_CFG_PATH=/etc/hyperledger/fabric
      - CORE_CHAINCODE_BUILDER=cp.icr.io/cp/ibp-ccenv:1.4.6-20200416-amd64
      - CORE_CHAINCODE_GOLANG_RUNTIME=cp.icr.io/cp/ibp-goenv:1.4.6-20200416-amd64
      - CORE_CHAINCODE_NODE_RUNTIME=cp.icr.io/cp/ibp-nodeenv:1.4.6-20200416-amd64
      - CORE_CHAINCODE_GOLANG_DYNAMICLINK=true
...
services:
  ...
  ca-org1:
    image: cp.icr.io/cp/ibp-ca:1.4.6-20200416-amd64
    environment:
      - LICENSE=accept
...
5. Change back into the main directory of the BYFN example and save the following Docker Compose file for the gRPC web proxy image as `docker-compose-proxy`:
  ...
  services:
    web_proxy.peer0.org1.example.com:
      ...
      image: cp.icr.io/cp/ibp-grpcweb:2.1.3-20200416-amd64
      environment:
        - LICENSE=accept
